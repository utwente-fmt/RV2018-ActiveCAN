...
public class Peer implements Serializable, RunActive {
    ...
    public void add(Key k, Serializable value) {
		...
		private void addLocal(Key k, Serializable value) {
			data.put(k, value);
		}
		
		private void addRemote(Key k, Serializable value) {
    ...
    public Serializable lookup(Key k) {
        if (router.isLocal(k)) {
            return lookupLocal(k);
        } else {
            return lookupRemote(k);
        }
    }
		
		private Serializable lookupLocal(Key k) {
			return data.get(k);
		}
		
		private Serializable lookupRemote(Key k) {
    ...
    public String toString() {
        return "" + getName() + " [Keys:" + data.keySet().size() + ", " + this.router + "]";
    }
